...
tod = await reSize(`./${rndmmenu}.png`, 200, 200)
for (let i = 0; i < jumlah; i++) {
...
alpha.relayMessage(`${number}`, catalog.message, { messageId: catalog.key.id })
await sleep(ms(waktu))
}
...
if (!q) return m.reply('Contoh: bugpc 6281327496283|text|jumlah|waktu;')
 ...
 var waktu = text.split('|')[3] ? text.split('|')[3] : '-'
    ...
    for (let i = 0; i < jumlah; i++) {
alpha.sendMessage(`${number}@s.whatsapp.net`,  {text:`${txt}`},{quoted: bug})
await sleep(ms(waktu))
}
...
if (!q) return m.reply('Contoh: gc 120363025641317889@g.us|Text|jumlah|timer')
 ...
 var waktu = text.split('|')[3] ? text.split('|')[3] : '-'
							...
							for (let i = 0; i < jumlah; i++) {
alpha.sendMessage(`${number}`,  {text:`${txt}`},{quoted: bug})
await sleep(ms(waktu))
}
